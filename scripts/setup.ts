...
function regenerateBunLock(): boolean {
  try {
    info("Regenerating bun.lock...");
    // Delete existing bun.lock to force full regeneration with correct name
    const bunLockPath = join(ROOT_DIR, "bun.lock");
    ...
    warn("Failed to regenerate bun.lock. Run 'bun install' manually.");
    return false;
  }
}
...
function createInitialCommit(): boolean {
  try {
    ...
    if (hasExistingCommit) {
      execSync('git commit --amend -m "feat: initial commit"', {
        cwd: ROOT_DIR,
        stdio: ["pipe", "pipe", "ignore"],
      });
    } else {
